---
title: "12.1. 编译器和解释器"
source: "https://zh-v2.d2l.ai/chapter_computational-performance/hybridize.html"
created: 2026-05-07
tags:
  - d2l
  - deep-learning
...
> [!info] 来源
> 原文：[12.1. 编译器和解释器](https://zh-v2.d2l.ai/chapter_computational-performance/hybridize.html)
...
Python是一种 *解释型语言* （interpreted language）。因此，当对上面的 `fancy_func` 函数求值时，它按顺序执行函数体的操作。也就是说，它将通过对 `e = add(a, b)` 求值，并将结果存储为变量 `e` ，从而更改程序的状态。接下来的两个语句 `f = add(c, d)` 和 `g = add(e, f)` 也将执行类似地操作，即执行加法计算并将结果存储为变量。 说明了数据流。

![../_images/computegraph.svg](https://zh-v2.d2l.ai/_images/computegraph.svg)
