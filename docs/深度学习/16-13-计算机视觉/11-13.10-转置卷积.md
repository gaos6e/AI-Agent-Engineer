---
title: "13.10. 转置卷积"
source: "https://zh-v2.d2l.ai/chapter_computer-vision/transposed-conv.html"
created: 2026-05-07
tags:
  - d2l
  - deep-learning
...
> [!info] 来源
> 原文：[13.10. 转置卷积](https://zh-v2.d2l.ai/chapter_computer-vision/transposed-conv.html)
> 项目：[动手学深度学习](https://github.com/d2l-ai/d2l-zh)
> 许可：[Apache-2.0](https://github.com/d2l-ai/d2l-zh/blob/master/LICENSE)

到目前为止，我们所见到的卷积神经网络层，例如卷积层（ [6.2节](https://zh-v2.d2l.ai/chapter_convolutional-neural-networks/conv-layer.html#sec-conv-layer) ）和汇聚层（ [6.5节](https://zh-v2.d2l.ai/chapter_convolutional-neural-networks/pooling.html#sec-pooling) ），通常会减少下采样输入图像的空间维度（高和宽）。 然而如果输入和输出图像的空间维度相同，在以像素级分类的语义分割中将会很方便。 例如，输出像素所处的通道维可以保有输入像素在同一位置上的分类结果。

为了实现这一点，尤其是在空间维度被卷积神经网络层缩小后，我们可以使用另一种类型的卷积神经网络层，它可以增加上采样中间层特征图的空间维度。 本节将介绍 *转置卷积* （transposed convolution） ([Dumoulin and Visin, 2016](https://zh-v2.d2l.ai/chapter_references/zreferences.html#id37 "Dumoulin, V., & Visin, F. (2016). A guide to convolution arithmetic for deep learning. arXiv preprint arXiv:1603.07285."))， 用于逆转下采样导致的空间尺寸减小。
...
例如， 解释了如何为 $2\times 2$ 的输入张量计算卷积核为 $2\times 2$ 的转置卷积。
...
与通过卷积核“减少”输入元素的常规卷积（在 [6.2节](https://zh-v2.d2l.ai/chapter_convolutional-neural-networks/conv-layer.html#sec-conv-layer) 中）相比，转置卷积通过卷积核“广播”输入元素，从而产生大于输入的输出。 我们可以通过 来构建输入张量 `X` 和卷积核张量 `K` 从而验证上述实现输出。 此实现是基本的二维转置卷积运算。
...
在转置卷积中，步幅被指定为中间结果（输出），而不是输入。 使用 中相同输入和卷积核张量，将步幅从1更改为2会增加中间张量的高和权重，因此输出张量在 中。

![../_images/trans_conv_stride2.svg](https://zh-v2.d2l.ai/_images/trans_conv_stride2.svg)

图13.10.2 卷积核为 $2\times 2$ ，步幅为2的转置卷积。阴影部分是中间张量的一部分，也是用于计算的输入和卷积核张量元素。 [¶](#id9 "Permalink to this image")

以下代码可以验证 中步幅为2的转置卷积的输出。
...
1. 在 中，卷积输入 `X` 和转置的卷积输出 `Z` 具有相同的形状。他们的数值也相同吗？为什么？
