---
title: "14.8. 来自Transformers的双向编码器表示（BERT）"
source: "https://zh-v2.d2l.ai/chapter_natural-language-processing-pretraining/bert.html"
created: 2026-05-07
tags:
  - d2l
  - deep-learning
...
> [!info] 来源
> 原文：[14.8. 来自Transformers的双向编码器表示（BERT）](https://zh-v2.d2l.ai/chapter_natural-language-processing-pretraining/bert.html)
...
在本章的其余部分，我们将深入了解BERT的训练前准备。当在 [15节](https://zh-v2.d2l.ai/chapter_natural-language-processing-applications/index.html#chap-nlp-app) 中解释自然语言处理应用时，我们将说明针对下游应用的BERT微调。
...
BERT选择Transformer编码器作为其双向架构。在Transformer编码器中常见是，位置嵌入被加入到输入序列的每个位置。然而，与原始的Transformer编码器不同，BERT使用 *可学习的* 位置嵌入。总之， 表明BERT输入序列的嵌入是词元嵌入、片段嵌入和位置嵌入的和。

![../_images/bert-input.svg](https://zh-v2.d2l.ai/_images/bert-input.svg)
...
如 [8.3节](https://zh-v2.d2l.ai/chapter_recurrent-neural-networks/language-models-and-dataset.html#sec-language-model) 所示，语言模型使用左侧的上下文预测词元。为了双向编码上下文以表示每个词元，BERT随机掩蔽词元并使用来自双向上下文的词元以自监督的方式预测掩蔽词元。此任务称为 *掩蔽语言模型* 。
...
值得注意的是，上述两个预训练任务中的所有标签都可以从预训练语料库中获得，而无需人工标注。原始的BERT已经在图书语料库 ([Zhu *et al.*, 2015](https://zh-v2.d2l.ai/chapter_references/zreferences.html#id198 "Zhu, Y., Kiros, R., Zemel, R., Salakhutdinov, R., Urtasun, R., Torralba, A., & Fidler, S. (2015). Aligning books and movies: towards story-like visual explanations by watching movies and reading books. Proceedings of the IEEE international conference on computer vision (pp. 19–27)."))和英文维基百科的连接上进行了预训练。这两个文本语料库非常庞大：它们分别有8亿个单词和25亿个单词。
...
3. 在BERT的原始实现中， `BERTEncoder` 中的位置前馈网络（通过 `d2l.EncoderBlock` ）和 `MaskLM` 中的全连接层都使用高斯误差线性单元（Gaussian error linear unit，GELU） ([Hendrycks and Gimpel, 2016](https://zh-v2.d2l.ai/chapter_references/zreferences.html#id63 "Hendrycks, D., & Gimpel, K. (2016). Gaussian error linear units (gelus). arXiv preprint arXiv:1606.08415."))作为激活函数。研究GELU与ReLU之间的差异。

Table Of Contents

[Previous](https://zh-v2.d2l.ai/chapter_natural-language-processing-pretraining/similarity-analogy.html)

[14.7. 词的相似性和类比任务](https://zh-v2.d2l.ai/chapter_natural-language-processing-pretraining/similarity-analogy.html)[Next

14.9. 用于预训练BERT的数据集

](https://zh-v2.d2l.ai/chapter_natural-language-processing-pretraining/bert-dataset.html)
