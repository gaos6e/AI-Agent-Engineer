---
title: "4.8. 数值稳定性和模型初始化"
source: "https://zh-v2.d2l.ai/chapter_multilayer-perceptrons/numerical-stability-and-init.html"
created: 2026-05-07
tags:
  - d2l
  - deep-learning
...
> [!info] 来源
> 原文：[4.8. 数值稳定性和模型初始化](https://zh-v2.d2l.ai/chapter_multilayer-perceptrons/numerical-stability-and-init.html)
...
(4.8.1) [¶](#equation-chapter-multilayer-perceptrons-numerical-stability-and-init-0 "Permalink to this equation") $\mathbf{h}^{(l)} = f_l (\mathbf{h}^{(l-1)}) \text{ 因此 } \mathbf{o} = f_L \circ \ldots \circ f_1(\mathbf{x}).$

如果所有隐藏变量和输入都是向量， 我们可以将 $\mathbf{o}$ 关于任何一组参数 $\mathbf{W}^{(l)}$ 的梯度写为下式：

(4.8.2) [¶](#equation-chapter-multilayer-perceptrons-numerical-stability-and-init-1 "Permalink to this equation") $\partial_{\mathbf{W}^{(l)}} \mathbf{o} = \underbrace{\partial_{\mathbf{h}^{(L-1)}} \mathbf{h}^{(L)}}_{ \mathbf{M}^{(L)} \stackrel{\mathrm{def}}{=}} \cdot \ldots \cdot \underbrace{\partial_{\mathbf{h}^{(l)}} \mathbf{h}^{(l+1)}}_{ \mathbf{M}^{(l+1)} \stackrel{\mathrm{def}}{=}} \underbrace{\partial_{\mathbf{W}^{(l)}} \mathbf{h}^{(l)}}_{ \mathbf{v}^{(l)} \stackrel{\mathrm{def}}{=}}.$
...
曾经sigmoid函数 $1/(1 + \exp(-x))$ （ [4.1节](https://zh-v2.d2l.ai/chapter_multilayer-perceptrons/mlp.html#sec-mlp) 提到过）很流行， 因为它类似于阈值函数。 由于早期的人工神经网络受到生物神经网络的启发， 神经元要么完全激活要么完全不激活（就像生物神经元）的想法很有吸引力。 然而，它却是导致梯度消失问题的一个常见的原因， 让我们仔细看看sigmoid函数为什么会导致梯度消失。
...
![../_images/output_numerical-stability-and-init_e60514_3_1.svg](https://zh-v2.d2l.ai/_images/output_numerical-stability-and-init_e60514_3_1.svg)
...
![../_images/output_numerical-stability-and-init_e60514_6_0.svg](https://zh-v2.d2l.ai/_images/output_numerical-stability-and-init_e60514_6_0.svg)
...
![../_images/output_numerical-stability-and-init_e60514_9_0.svg](https://zh-v2.d2l.ai/_images/output_numerical-stability-and-init_e60514_9_0.svg)
...
![../_images/output_numerical-stability-and-init_e60514_12_0.svg](https://zh-v2.d2l.ai/_images/output_numerical-stability-and-init_e60514_12_0.svg)
...
(4.8.3) [¶](#equation-chapter-multilayer-perceptrons-numerical-stability-and-init-2 "Permalink to this equation") $o_{i} = \sum_{j=1}^{n_\mathrm{in}} w_{ij} x_j.$
...
(4.8.4) [¶](#equation-chapter-multilayer-perceptrons-numerical-stability-and-init-3 "Permalink to this equation") $\begin{split}\begin{aligned}
...
(4.8.5) [¶](#equation-chapter-multilayer-perceptrons-numerical-stability-and-init-4 "Permalink to this equation") $\begin{aligned}
\frac{1}{2} (n_\mathrm{in} + n_\mathrm{out}) \sigma^2 = 1 \text{ 或等价于 }
\sigma = \sqrt{\frac{2}{n_\mathrm{in} + n_\mathrm{out}}}.
\end{aligned}$

这就是现在标准且实用的 *Xavier初始化* 的基础， 它以其提出者 ([Glorot and Bengio, 2010](https://zh-v2.d2l.ai/chapter_references/zreferences.html#id44 "Glorot, X., & Bengio, Y. (2010). Understanding the difficulty of training deep feedforward neural networks. Proceedings of the thirteenth international conference on artificial intelligence and statistics (pp. 249–256).")) 第一作者的名字命名。 通常，Xavier初始化从均值为零，方差 $\sigma^2 = \frac{2}{n_\mathrm{in} + n_\mathrm{out}}$ 的高斯分布中采样权重。 我们也可以将其改为选择从均匀分布中抽取权重时的方差。 注意均匀分布 $U(-a, a)$ 的方差为 $\frac{a^2}{3}$ 。 将 $\frac{a^2}{3}$ 代入到 $\sigma^2$ 的条件中，将得到初始化值域：

(4.8.6) [¶](#equation-chapter-multilayer-perceptrons-numerical-stability-and-init-5 "Permalink to this equation") $U\left(-\sqrt{\frac{6}{n_\mathrm{in} + n_\mathrm{out}}}, \sqrt{\frac{6}{n_\mathrm{in} + n_\mathrm{out}}}\right).$
...
4. 如果我们知道某些项是发散的，我们能在事后修正吗？看看关于按层自适应速率缩放的论文 ([You *et al.*, 2017](https://zh-v2.d2l.ai/chapter_references/zreferences.html#id192 "You, Y., Gitman, I., & Ginsburg, B. (2017). Large batch training of convolutional networks. arXiv preprint arXiv:1708.03888.")) 。

Table Of Contents

[Previous](https://zh-v2.d2l.ai/chapter_multilayer-perceptrons/backprop.html)

[4.7. 前向传播、反向传播和计算图](https://zh-v2.d2l.ai/chapter_multilayer-perceptrons/backprop.html)[Next
