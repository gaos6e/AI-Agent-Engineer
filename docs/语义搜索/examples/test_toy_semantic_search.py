...
if SPEC is None or SPEC.loader is None:
    raise RuntimeError("无法加载 toy_semantic_search.py")
...
class ContractAndMathTests(unittest.TestCase):
    ...
    def test_rrf_rejects_duplicate_or_single_channel(self) -> None:
        ...
        with self.assertRaisesRegex(lab.SemanticSearchError, "至少需要两个"):
...
class FixtureValidationTests(TemporaryFixtureMixin, unittest.TestCase):
    ...
    def test_duplicate_key_and_nonfinite_number_are_rejected(self) -> None:
        ...
        with self.assertRaisesRegex(lab.SemanticSearchError, "重复字段"):
            lab.load_fixture(self.path)
        self.path.write_text(
            '{"schema_version":NaN}',
            encoding="utf-8",
        )
        with self.assertRaisesRegex(lab.SemanticSearchError, "非有限"):
            lab.load_fixture(self.path)

    def test_exact_top_fields_and_schema_version_are_checked(self) -> None:
        value = raw_fixture()
        value["extra"] = True
        with self.assertRaisesRegex(lab.SemanticSearchError, "字段必须精确"):
    ...
    def test_document_status_acl_and_duplicate_id_are_checked(self) -> None:
        ...
        with self.assertRaisesRegex(lab.SemanticSearchError, "字典序"):
            lab.load_fixture(self.write(value))
        value = raw_fixture()
        value["documents"][1]["id"] = value["documents"][0]["id"]
        with self.assertRaisesRegex(lab.SemanticSearchError, "document id 重复"):
            lab.load_fixture(self.write(value))
    ...
    def test_query_groups_filters_and_relevance_are_checked(self) -> None:
        ...
        with self.assertRaisesRegex(lab.SemanticSearchError, "不允许字段"):
            lab.load_fixture(self.write(value))
        value = raw_fixture()
    ...
    def test_qrels_must_exist_be_eligible_and_not_overlap_denials(self) -> None:
        ...
        with self.assertRaisesRegex(lab.SemanticSearchError, "未知文档"):
            lab.load_fixture(self.write(value))
        value = raw_fixture()
        value["queries"][0]["qrels"] = {"d-09-beta-private": 3}
        with self.assertRaisesRegex(lab.SemanticSearchError, "不满足权限"):
            lab.load_fixture(self.write(value))
        value = raw_fixture()
        value["queries"][1]["qrels"] = {"d-09-beta-private": 3}
        with self.assertRaisesRegex(lab.SemanticSearchError, "重叠"):
            lab.load_fixture(self.write(value))
    ...
    def test_denied_documents_must_exist_and_be_ineligible(self) -> None:
        ...
        with self.assertRaisesRegex(lab.SemanticSearchError, "仍满足过滤"):
            lab.load_fixture(self.write(value))

    def test_duplicate_query_id_is_rejected(self) -> None:
        value = raw_fixture()
        value["queries"][1]["id"] = value["queries"][0]["id"]
        with self.assertRaisesRegex(lab.SemanticSearchError, "query id 重复"):
            lab.load_fixture(self.write(value))
