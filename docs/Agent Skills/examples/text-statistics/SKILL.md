...
description: Count word-like tokens, Unicode characters, and logical lines with deterministic JSON output. Use when a user asks for text-size statistics, word or line counts, or a machine-readable count summary; do not use for rewriting, translation, sentiment, token billing, or semantic analysis.
license: CC0-1.0
compatibility: Requires Python 3; reads only an explicit UTF-8 input and does not access the network.
metadata:
  version: 1.0.0
...
2. Prefer `scripts/text_stats.py --input <UTF8_FILE>` for file content. Use `--text <TEXT>` only for short, non-secret text because command histories can retain arguments.
...
```powershell
python -B scripts/text_stats.py --text "Hello Agent 世界"
python -B scripts/text_stats.py --input .\sample.txt
```
