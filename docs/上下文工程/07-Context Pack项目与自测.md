...
| [[上下文工程/examples/test_context_budget.py\|test_context_budget.py]] | 15 项单元测试，覆盖非法输入、失败关闭、确定性、schema 和 CLI |

## 运行环境

脚本只使用 Python 标准库，不需要联网、第三方依赖或 API key。在 Windows 11 的 PowerShell 7 中执行：

```powershell
cd "X:\path\to\your-vault\Knowledge\AI Agent Engineer\docs\上下文工程"
py -3.11 --version
py -3.11 -m unittest -v .\examples\test_context_budget.py
py -3.11 -O -m unittest -v .\examples\test_context_budget.py
```
...
`dedupe_key` 只应用于业务上确认可互换的片段。相互矛盾的来源不能为了省 token 共用一个 key，否则会把冲突误当重复。

### 2. 构建默认 pack

```powershell
py -3.11 .\examples\context_budget.py
$LASTEXITCODE
```
...
```powershell
$pack = Join-Path $env:TEMP "context-pack.json"
py -3.11 .\examples\context_budget.py --json-pack $pack
Get-Content -LiteralPath $pack -Raw -Encoding utf8
```
...
- `python -O` 模式：同一 15 项测试全部通过，关键门禁不依赖可被优化移除的裸 `assert`。
...
- [ ] 我能运行普通与 `-O` 测试，并解释 15 项测试覆盖的失败类型。
...
- [OpenAI：Compaction](https://developers.openai.com/api/docs/guides/compaction)（访问于 2026-07-14）
