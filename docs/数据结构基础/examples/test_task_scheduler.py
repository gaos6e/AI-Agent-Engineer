...
class TaskSchedulerTests(unittest.TestCase):
    def test_task_collection_type_is_validated(self) -> None:
        ...
        for value in invalid_values:
            with self.subTest(value=value):
                with self.assertRaisesRegex(ScheduleInputError, "任务集合必须"):
                    schedule(value)
    ...
    def test_one_shot_iterator_is_rejected_without_consumption(self) -> None:
        ...
        with self.assertRaisesRegex(ScheduleInputError, "list 或 tuple"):
            schedule(one_shot)
    ...
    def test_duplicate_id_is_rejected(self) -> None:
        ...
        with self.assertRaisesRegex(ScheduleInputError, "id 重复"):
            schedule(tasks)

    def test_missing_dependency_is_rejected(self) -> None:
        tasks = [{"id": "a", "priority": 1, "depends_on": ["missing"]}]
        with self.assertRaisesRegex(ScheduleInputError, "依赖不存在"):
            schedule(tasks)
    ...
    def test_duplicate_dependency_is_rejected(self) -> None:
        ...
        with self.assertRaisesRegex(ScheduleInputError, "重复依赖"):
            schedule(tasks)

    def test_direct_self_dependency_is_rejected(self) -> None:
        tasks = [{"id": "a", "priority": 1, "depends_on": ["a"]}]
        with self.assertRaisesRegex(ScheduleInputError, "直接依赖自身"):
            schedule(tasks)
    ...
    def test_cycle_reports_blocked_not_cycle_members(self) -> None:
        ...
        with self.assertRaisesRegex(
            ScheduleInputError,
            "剩余任务被一个或多个依赖环阻塞: a, b, downstream",
        ):
            schedule(tasks)

    def test_non_mapping_task_is_rejected(self) -> None:
        with self.assertRaisesRegex(ScheduleInputError, "第 0 项必须是映射对象"):
            schedule(["not-a-task"])

    def test_missing_and_unknown_fields_are_rejected(self) -> None:
        cases = [
            ({"id": "a", "priority": 1}, "缺少字段"),
            (
    ...
    def test_field_names_must_be_strings(self) -> None:
        ...
        with self.assertRaisesRegex(ScheduleInputError, "字段名必须是内建 str"):
            schedule([task])
