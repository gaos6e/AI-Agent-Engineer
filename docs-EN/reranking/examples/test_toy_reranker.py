...
if SPEC is None or SPEC.loader is None:
    raise RuntimeError("could not load toy_reranker.py")
...
class MathAndContractTests(unittest.TestCase):
    ...
    def test_analyzer_normalises_ascii_and_builds_cjk_bigrams(self) -> None:
        # Keep this CJK sample: it exercises the analyzer's CJK-bigram branch.
        tokens = lab.analyze("Ｅ０４２ 漢字測試")
        self.assertEqual(tokens[0], "e042")
        self.assertIn("漢字", tokens)
        self.assertIn("測試", tokens)
...
class FixtureValidationTests(TemporaryFixtureMixin, unittest.TestCase):
    ...
    def test_duplicate_key_and_nonfinite_json_are_rejected(self) -> None:
        ...
        with self.assertRaisesRegex(lab.RerankerError, "duplicate JSON field"):
            lab.load_fixture(self.path)
        self.path.write_text('{"schema_version":NaN}', encoding="utf-8")
        with self.assertRaisesRegex(lab.RerankerError, "non-finite"):
            lab.load_fixture(self.path)

    def test_top_fields_and_schema_version_are_exact(self) -> None:
        value = raw_fixture()
        value["extra"] = True
        with self.assertRaisesRegex(lab.RerankerError, "fields must be exactly"):
    ...
    def test_query_date_groups_and_authorization_revision_are_checked(self) -> None:
        ...
        with self.assertRaisesRegex(lab.RerankerError, "lexicographically"):
            lab.load_fixture(self.write(value))
        value = raw_fixture()
        value["query"]["authorization_revision"] = ""
        with self.assertRaisesRegex(lab.RerankerError, "nonempty string"):
            lab.load_fixture(self.write(value))
    ...
    def test_settings_require_positive_values_and_top_not_above_window(self) -> None:
        ...
        with self.assertRaisesRegex(lab.RerankerError, "must not exceed"):
            lab.load_fixture(self.write(value))
    ...
    def test_candidate_ids_and_ranks_are_unique_and_contiguous(self) -> None:
        ...
        with self.assertRaisesRegex(lab.RerankerError, "duplicate candidate id"):
            lab.load_fixture(self.write(value))
        value = raw_fixture()
        value["candidates"][1]["first_rank"] = 1
        with self.assertRaisesRegex(lab.RerankerError, "duplicate first_rank"):
            lab.load_fixture(self.write(value))
        value = raw_fixture()
        value["candidates"][8]["first_rank"] = 10
        with self.assertRaisesRegex(lab.RerankerError, "contiguous"):
            lab.load_fixture(self.write(value))

    def test_qrels_must_exist_be_eligible_and_use_integer_grades(self) -> None:
        value = raw_fixture()
        value["qrels"] = {"unknown": 3}
        with self.assertRaisesRegex(lab.RerankerError, "unknown"):
            lab.load_fixture(self.write(value))
        value = raw_fixture()
        value["candidates"][0]["status"] = "draft"
        value["qrels"] = {"d-01-membership": 3}
        with self.assertRaisesRegex(lab.RerankerError, "fails hard filtering"):
            lab.load_fixture(self.write(value))
        value = raw_fixture()
        value["qrels"]["d-02-refund-apply"] = True
        with self.assertRaisesRegex(lab.RerankerError, "from 1 to 3"):
            lab.load_fixture(self.write(value))
    ...
    def test_denied_candidates_must_exist_be_ineligible_and_not_overlap_qrels(self) -> None:
        ...
        with self.assertRaisesRegex(lab.RerankerError, "is still eligible"):
            lab.load_fixture(self.write(value))
        value = raw_fixture()
        value["must_not_return"].append("d-02-refund-apply")
        value["must_not_return"].sort()
        with self.assertRaisesRegex(lab.RerankerError, "must not overlap"):
            lab.load_fixture(self.write(value))
    ...
    def test_fingerprints_bind_every_normalized_security_and_scoring_input(self) -> None:
        ...
        mutations = {
            ...
            "query_text": lambda value: value["query"].__setitem__(
                "text", "How long does an approved refund take to reach the original payment method?"
            ),
            ...
            "candidate_title": lambda value: value["candidates"][0].__setitem__(
                "title", "Revised membership renewal rules"
            ),
            "candidate_text": lambda value: value["candidates"][0].__setitem__(
                "text", "Automatic renewal can be disabled before a charge is made."
            ),
