"""Fit y = w*x + b with the standard library and verify least-squares orthogonality."""
...
def fit_line(xs: Sequence[float], ys: Sequence[float]) -> LineFit:
    ...
    if len(x_values) != len(y_values) or len(x_values) < 2:
        raise ValueError("xs and ys must have equal length and at least two points")
    ...
    if denominator == 0:
        raise ValueError("all x values are equal, so the slope is not identifiable")
...
def main() -> int:
    ...
    if abs(result.residual_sum) > tolerance:
        raise RuntimeError("residual sum failed the orthogonality check")
    if abs(result.centered_residual_dot) > tolerance:
        raise RuntimeError("centered x and residuals failed the orthogonality check")
    return 0
