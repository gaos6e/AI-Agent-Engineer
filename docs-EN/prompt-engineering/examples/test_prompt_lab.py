...
class PromptLabTests(unittest.TestCase):
    ...
    def test_prompt_and_schema_versions_are_bound_to_runtime_assets(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaisesRegex(ValueError, "prompt_version"):
                prompt_lab.render_messages("sign-in failed", "ticket-router-9.9.9")
    ...
    def test_exact_fields_label_and_reason_are_enforced(self) -> None:
        ...
        responses = (
            ...
            ('{"label":"secret","reason":"ok","evidence":"credit card"}', "unsupported"),
            ('{"label":[],"reason":"ok","evidence":"credit card"}', "unsupported"),
            ('{"label":"billing","reason":"","evidence":"credit card"}', "non-blank"),
            (
                json.dumps(
                    {"label": "billing", "reason": "x" * 161, "evidence": "credit card"},
                    ensure_ascii=False,
                ),
                "exceeds",
            ),
            (
                json.dumps(
                    {"label": "billing", "reason": " " + "x" * 160, "evidence": "credit card"},
                    ensure_ascii=False,
                ),
                "exceeds",
            ),
    ...
    def test_evidence_must_be_grounded_and_required_for_specific_labels(self) -> None:
        case = prompt_lab.load_case_set(self.cases_path).cases[0]
        ungrounded = '{"label":"billing","reason":"ok","evidence":"evidence that is not present"}'
    ...
    def test_cli_returns_failure_for_broken_mock_response(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            raw["cases"][0]["mock_response"] = (
                '{"label":"technical","reason":"wrong","evidence":"credit card"}'
            )
