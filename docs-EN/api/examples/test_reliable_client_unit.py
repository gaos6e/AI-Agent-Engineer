"""Use a scripted Session to verify waiting, timeout, and error boundaries precisely."""
...
class ScriptedSession:
    ...
    def request(self, method: str, url: str, **kwargs: Any) -> requests.Response:
        ...
        if not self.outcomes:
            raise AssertionError("ScriptedSession has no remaining response")
...
class ReliableApiClientUnitTests(unittest.TestCase):
    ...
    def test_http_date_retry_after_uses_injected_reference_time(self) -> None:
        ...
        self.assertEqual(ReliableApiClient._parse_retry_after(future, now=now), 12.0)
        self.assertEqual(ReliableApiClient._parse_retry_after(past, now=now), 0.0)
        self.assertIsNone(
            ReliableApiClient._parse_retry_after("14 Jul 2026", now=now)
        )
    ...
    def test_tls_error_is_normalized_without_retry(self) -> None:
        """SSLError subclasses ConnectionError and must not reach the retryable transport branch."""
    ...
    def test_constructor_rejects_invalid_configuration(self) -> None:
        invalid_cases = [
            ...
            ({"base_url": "https://example.test", "connect_timeout": 0}, "positive"),
            ({"base_url": "https://example.test", "read_timeout": float("inf")}, "finite"),
            ({"base_url": "https://example.test", "jitter_ratio": 1.1}, "between 0 and 1"),
        ]
    ...
    def test_idempotency_key_and_pagination_bounds_are_validated(self) -> None:
        session = ScriptedSession()
        client = ReliableApiClient("https://api.example.test", session=session)
        for key in ("", " key", "key ", "two words", "line\nbreak", "action–one", 3, None):
