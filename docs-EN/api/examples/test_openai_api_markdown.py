"""Offline contract checks for Python snippets in the OpenAI API lesson."""
...
LESSON_PATH = Path(__file__).parents[1] / "ai-api-reference" / "openai-api.md"
...
class OpenAIMarkdownContractTests(unittest.TestCase):
    ...
    def test_file_example_has_expiry_and_final_cleanup(self) -> None:
        ...
        self.assertIn('require_text(response, context="file analysis")', block)
    ...
    def test_text_commit_rejects_mixed_text_and_refusal(self) -> None:
        ...
        exec(
            compile(ast.fix_missing_locations(helpers), "<openai-helpers>", "exec"),
            namespace,
        )
        ...
        with self.assertRaisesRegex(RuntimeError, "contains a refusal"):
