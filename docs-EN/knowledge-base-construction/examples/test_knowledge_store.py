...
class KnowledgeStoreTests(unittest.TestCase):
    ...
    def record(
        ...
        *,
        ...
        sequence: int = 1,
        version: str = "v1",
        content: str = "Leave policy requires advance submission.",
    ...
    def test_older_source_event_is_ignored(self) -> None:
        ...
        result = subject.upsert_record(
            self.connection,
            self.record(sequence=1, content="An older event must not overwrite the current record."),
            self.config,
            run_id="late-run",
        )

        self.assertEqual("stale_ignored", result.action)
        self.assertEqual("Leave policy requires advance submission.", self.search()[0]["content"])
    ...
    def test_same_sequence_with_different_source_state_is_conflict(self) -> None:
        ...
        with self.assertRaisesRegex(subject.StoreError, "same source_sequence"):
            subject.upsert_record(
                self.connection,
                self.record(content="Conflicting content"),
                self.config,
                run_id="run-conflict",
            )
        self.assertEqual(1, self.connection.execute("SELECT count(*) FROM revisions").fetchone()[0])
    ...
    def test_content_update_keeps_old_published_revision_until_success(self) -> None:
        ...
        updated = self.record(
            sequence=2,
            version="v2",
            content="Leave policy requires submission two days in advance.",
        )
    ...
    def test_projection_failure_preserves_old_version_and_is_retryable(self) -> None:
        ...
        result = subject.upsert_record(
            self.connection,
            self.record(sequence=2, version="v2", content="Leave policy, version two."),
            self.config,
            run_id="run-2",
        )
    ...
    def test_acl_change_blocks_old_projection_before_new_acl_is_published(self) -> None:
        self.insert_and_publish(self.record(groups=("admins",), content="Internal security policy"))
        changed = self.record(
            sequence=2,
            version="v2",
            groups=("security",),
        ...
        self.assertEqual([], self.search(groups=("admins",), term="security"))
        self.assertEqual([], self.search(groups=("security",), term="security"))
        subject.project_next_event(self.connection)
        self.assertEqual([], self.search(groups=("admins",), term="security"))
        self.assertEqual("policy:leave", self.search(groups=("security",), term="security")[0]["document_id"])
    ...
    def test_same_sequence_delete_reason_conflict_is_rejected(self) -> None:
        ...
        with self.assertRaisesRegex(subject.StoreError, "delete reason conflicts"):
            subject.delete_document(
                self.connection,
                "tenant-a",
                "policy:leave",
    ...
    def test_same_sequence_cannot_be_upsert_and_delete(self) -> None:
        ...
        with self.assertRaisesRegex(subject.StoreError, "cannot represent both upsert and delete"):
            subject.delete_document(
                self.connection,
                "tenant-a",
                "policy:leave",
    ...
    def test_resurrection_requires_newer_sequence_and_waits_for_projection(self) -> None:
        ...
        with self.assertRaisesRegex(subject.StoreError, "cannot resurrect"):
            subject.upsert_record(
                self.connection,
                self.record(sequence=2),
                self.config,
    ...
    def test_newline_and_nfc_normalisation_make_equivalent_retry_noop(self) -> None:
        # Keep this decomposed/composed accent pair: it verifies Unicode and newline normalization.
    ...
    def test_content_size_limit_is_enforced(self) -> None:
        ...
        with self.assertRaisesRegex(subject.StoreError, "must not exceed"):
            subject.upsert_record(
    ...
    def test_outbox_revision_identity_mismatch_is_rejected_before_projection(self) -> None:
        ...
        with self.assertRaisesRegex(RuntimeError, "event identity"):
            subject.project_next_event(self.connection)
    ...
    def test_query_fails_closed_on_search_body_tamper(self) -> None:
        self.insert_and_publish()
        self.connection.execute(
            "UPDATE search_revisions SET content = 'sensitive leave projection'"
        )

        with self.assertRaisesRegex(subject.StoreError, "search_content_hash_mismatch"):
            self.search(term="sensitive")
    ...
    def test_query_fails_closed_on_projection_acl_grant_tamper(self) -> None:
        ...
        self.assertEqual([], self.search(groups=("guests",), term="security"))
        report = subject.reconcile(self.connection)
    ...
    def test_unauthorized_candidates_do_not_consume_query_limit(self) -> None:
        subject.upsert_record(
            ...
            self.record(
                document="a-private",
                content="shared search term private body",
                groups=("admins",),
            ),
            self.config,
            run_id="private",
        ...
        subject.upsert_record(
            ...
            self.record(
                document="z-public",
                content="shared search term public body",
                groups=("employees",),
            ),
            self.config,
            run_id="public",
        ...
        result = subject.search_visible(
            ...
            term="shared search term",
            limit=1,
        )
        self.assertEqual(["z-public"], [item["document_id"] for item in result])
        self.assertNotIn("private body", json.dumps(result, ensure_ascii=False))
    ...
    def test_query_recomputes_canonical_acl_state_hash(self) -> None:
        ...
        with self.assertRaisesRegex(
            ...
            self.search(groups=("guests",), term="security")
        report = subject.reconcile(self.connection)
    ...
    def test_query_and_acl_resource_limits_fail_closed(self) -> None:
        ...
        subject.drain_outbox(self.connection)
        with self.assertRaisesRegex(subject.StoreError, "candidate window"):
    ...
    def test_query_defends_against_cross_tenant_pointer_corruption(self) -> None:
        ...
        other = self.record(
            ...
            content="Confidential leave policy",
        )
        subject.upsert_record(self.connection, other, self.config, run_id="run-b")
        subject.drain_outbox(self.connection)
    ...
    def test_purge_waits_for_delete_projection_then_nulls_canonical_content(self) -> None:
        ...
        with self.assertRaisesRegex(subject.StoreError, "propagation has not completed"):
            subject.purge_deleted_canonical_content(
                self.connection, tenant_id="tenant-a", document_id="policy:leave"
            )
        subject.drain_outbox(self.connection)
