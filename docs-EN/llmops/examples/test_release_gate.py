"""Minimal regression tests for the offline release gate; uses only the Python standard library."""
...
class ReleaseGateTests(unittest.TestCase):
    ...
    def test_unknown_manifest_field_is_rejected(self) -> None:
        ...
        with self.assertRaisesRegex(release_gate.ContractError, "unknown fields"):
    ...
    def test_missing_component_is_rejected(self) -> None:
        ...
        with self.assertRaisesRegex(release_gate.ContractError, "is missing fields: pricing"):
            release_gate.validate_manifest(malformed)

    def test_mutable_model_alias_is_rejected(self) -> None:
        malformed = copy.deepcopy(self.manifest)
        malformed["candidates"][0]["model"]["model_snapshot"] = "latest"
        with self.assertRaisesRegex(release_gate.ContractError, "non-floating identifier"):
            release_gate.validate_manifest(malformed)
        ...
        for value in (
            ...
            with self.subTest(value=value):
                ...
                with self.assertRaisesRegex(
                    ...
                    r"model_snapshot.*non-floating identifier",
                ):
                    release_gate.validate_manifest(malformed)
    ...
    def test_floating_branch_ref_is_rejected_for_app_commit(self) -> None:
        for value in (
            ...
            with self.subTest(value=value):
                ...
                with self.assertRaisesRegex(
                    release_gate.ContractError,
                    r"app\.commit.*non-floating identifier",
                ):
                    release_gate.validate_manifest(malformed)
    ...
    def test_duplicate_release_id_is_rejected(self) -> None:
        ...
        with self.assertRaisesRegex(release_gate.ContractError, "duplicate release_id"):
            release_gate.validate_manifest(malformed)
    ...
    def test_boolean_is_not_accepted_as_number(self) -> None:
        ...
        with self.assertRaisesRegex(release_gate.ContractError, "must be numeric"):
            release_gate.validate_manifest(malformed)

    def test_fractional_sample_count_is_rejected(self) -> None:
        malformed = copy.deepcopy(self.manifest)
        malformed["candidates"][0]["metrics"]["safety_sample_count"] = 50.5
        with self.assertRaisesRegex(release_gate.ContractError, "must be an integer"):
            release_gate.validate_manifest(malformed)
    ...
    def test_rate_above_one_is_rejected(self) -> None:
        ...
        with self.assertRaisesRegex(release_gate.ContractError, "at most 1"):
    ...
    def test_non_finite_metric_is_rejected(self) -> None:
        ...
        with self.assertRaisesRegex(release_gate.ContractError, "finite"):
            release_gate.validate_manifest(malformed)

    def test_overflowing_integer_is_a_contract_error(self) -> None:
        malformed = copy.deepcopy(self.manifest)
        malformed["baseline"]["metrics"]["p95_latency_ms"] = 10**400
        with self.assertRaisesRegex(release_gate.ContractError, "representable as a finite number"):
            release_gate.validate_manifest(malformed)
    ...
    def test_nonstandard_json_nan_is_rejected_during_load(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            with self.assertRaisesRegex(release_gate.ContractError, "outside the JSON standard"):
    ...
    def test_non_utf8_input_is_a_contract_error_at_load_and_digest(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            with self.assertRaisesRegex(release_gate.ContractError, "valid UTF-8 JSON"):
                release_gate.load_json(path)
        with self.assertRaisesRegex(release_gate.ContractError, "Unicode surrogate"):
    ...
    def test_duplicate_json_key_is_rejected_during_load(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            with self.assertRaisesRegex(release_gate.ContractError, "duplicate field"):
                release_gate.load_json(path)

    def test_evaluation_artifact_is_bound_to_release(self) -> None:
        malformed = copy.deepcopy(self.manifest)
        malformed["candidates"][0]["evaluation"]["subject_release_id"] = "other"
        with self.assertRaisesRegex(release_gate.ContractError, "bind its owning release_id"):
            release_gate.validate_manifest(malformed)
    ...
    def test_evaluation_artifact_requires_full_sha256(self) -> None:
        ...
        with self.assertRaisesRegex(release_gate.ContractError, "64-character"):
            release_gate.validate_manifest(malformed)
    ...
    def test_evaluation_artifact_requires_a_supported_digest_format(self) -> None:
        ...
        with self.assertRaisesRegex(release_gate.ContractError, "supported evidence digest format"):
            release_gate.validate_manifest(malformed)
    ...
    def test_fallback_manifest_cannot_point_to_candidate_itself(self) -> None:
        ...
        with self.assertRaisesRegex(release_gate.ContractError, "must not point to itself"):
            release_gate.validate_manifest(malformed)
    ...
    def test_duplicate_tool_name_is_rejected(self) -> None:
        ...
        with self.assertRaisesRegex(release_gate.ContractError, "duplicate name"):
            release_gate.validate_manifest(malformed)
    ...
    def test_known_blocked_candidate_cannot_be_fallback(self) -> None:
        ...
        with self.assertRaisesRegex(release_gate.ContractError, "did not pass the candidate gate"):
            release_gate.validate_manifest(malformed)
    ...
    def test_known_fallback_candidates_cannot_form_cycle(self) -> None:
        ...
        with self.assertRaisesRegex(release_gate.ContractError, "forms a cycle"):
            release_gate.validate_manifest(malformed)
    ...
    def test_online_arm_counts_must_be_consistent(self) -> None:
        ...
        with self.assertRaisesRegex(release_gate.ContractError, "must not exceed eligible_count"):
            release_gate.validate_observations(malformed)
    ...
    def test_online_arm_outcome_numerators_must_match_denominators(self) -> None:
        ...
        with self.assertRaisesRegex(release_gate.ContractError, "must not exceed labeled_count"):
            release_gate.validate_observations(malformed)
    ...
    def test_regression_candidate_is_blocked_for_multiple_independent_reasons(self) -> None:
        ...
        self.assertIn("critical-slice success rate", joined)
        self.assertIn("safety-slice sample count", joined)
        self.assertIn("estimated cost", joined)
    ...
    def test_observation_rejects_mismatched_full_candidate_gate_evidence(self) -> None:
        ...
        with self.assertRaisesRegex(release_gate.ContractError, "full candidate-gate evidence digest does not match"):
    ...
    def test_online_quality_uses_concurrent_control_not_offline_baseline(self) -> None:
        ...
        self.assertIn("concurrent-control", "\n".join(decision.reasons))
    ...
    def test_provider_drift_uses_prevalidated_fallback(self) -> None:
        ...
        self.assertIn("fallback-path release", "\n".join(decision.reasons))
    ...
    def test_provider_drift_with_insufficient_candidate_evidence_does_not_fallback(self) -> None:
        ...
        self.assertIn("cannot switch automatically", "\n".join(decision.reasons))
    ...
    def test_quality_or_safety_regression_rolls_back(self) -> None:
        ...
        joined = "\n".join(decision.reasons)
        self.assertIn("task-success rate", joined)
        self.assertIn("safety violations", joined)
    ...
    def test_cli_keeps_insufficient_provider_evidence_in_human_review(self) -> None:
        ...
        self.assertIn("cannot switch automatically", stdout.getvalue())
    ...
    def test_audit_release_id_requires_at_least_one_bound_observation(self) -> None:
        ...
        self.assertEqual(2, code)
        self.assertIn("must bind at least one online observation", stderr.getvalue())
    ...
    def test_batch_audit_requires_observation_for_each_promoted_candidate(self) -> None:
        ...
        self.assertEqual(2, code)
        self.assertIn("lacks an online observation", stderr.getvalue())
    ...
    def test_observation_requires_recorded_candidate_promotion(self) -> None:
        ...
        with self.assertRaisesRegex(release_gate.ContractError, "with no promoted_at"):
