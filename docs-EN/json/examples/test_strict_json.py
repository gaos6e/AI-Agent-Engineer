...
class StrictJsonTests(unittest.TestCase):
    ...
    def test_chinese_escapes_and_windows_path_round_trip(self) -> None:
        value = {"title": "meeting", "path": "D:\\data\\input.json", "line": "a\nb"}
    ...
    def test_atomic_json_write_replaces_complete_document(self) -> None:
        with TemporaryDirectory() as directory:
            ...
            write_json_atomic(path, {"name": "meeting assistant", "ok": True})
            payload = path.read_bytes()
            self.assertFalse(payload.startswith(b"\xef\xbb\xbf"))
            self.assertTrue(payload.endswith(b"\n"))
            self.assertEqual(load_json_file(path), {"name": "meeting assistant", "ok": True})
