...
translation_route: en/machine-learning/transformer-models
...
下面的图保留原始 Transformer 的 **encoder-decoder** 数据流与 attention 公式，但为便于读图，采用的是常见的 **pre-norm** 表达：每个子层前先做 LayerNorm。原论文的 `Add & Norm` 则是子层输出先与残差相加、再 LayerNorm（post-norm）。因此它用于理解信息流，不应被当作 2017 原论文的逐行实现；dropout、padding mask 和训练工程细节也被省略。现代 decoder-only 大语言模型可以看成主要保留右侧 decoder 的 masked self-attention 和 MLP，并去掉 encoder / cross-attention。
...
3. **Encoder LayerNorm**：本图采用 pre-norm，在进入 self-attention 前稳定每个 token 的通道尺度。原论文采用 post-norm，即每个子层完成残差相加后再归一化；两者都服务于深层训练稳定性，但不是同一计算顺序。
...
7. **Encoder 残差与 FFN**：attention 输出与原输入相加，再由下一处 pre-norm 进入 FFN。attention 负责 token 间交互，FFN 负责每个 token 内部的非线性特征变换；若复现原论文，应把 LayerNorm 放在每个残差相加之后。
...
12. **Decoder 第一段残差**：masked self-attention 的结果加回目标端输入，保留原始目标历史表示；图中的下一处 LayerNorm 延续 pre-norm 顺序。

13. **Cross-Attention**：decoder 用自己的隐藏状态生成 query，用 encoder memory 生成 key/value。它的作用是让当前正在生成的目标 token 去源序列中查找相关信息。

14. **Decoder 第二段残差**：cross-attention 汇聚到的源端信息加回 decoder 表示，使目标端同时包含“已生成历史”和“源序列上下文”；图中的下一处 LayerNorm 延续 pre-norm 顺序。
...
在现代大模型中，具体顺序常从原论文的 post-norm 改为 pre-norm；激活函数、归一化、位置编码、attention 变体也会有很多工程改造。但核心仍是 attention + MLP + 残差堆叠。比较两个模型时，应先核对 norm 顺序、位置机制、mask、训练目标和实际推理路径，而不只比较“是否使用 Transformer”。
...
若还不能说明 logits、交叉熵、训练/验证/测试与梯度更新之间的关系，应先回到 [[机器学习/00-目录|机器学习]] 主线或 [[深度学习/00-目录|深度学习]] 的训练闭环；理解架构图不等于能够训练、评估或安全部署模型。

## 参考资料

核验日期：**2026-07-22**。本页使用原论文解释经典架构；现代框架的实现细节、默认值和性能结论必须以所选实现的当前文档和实验为准。
