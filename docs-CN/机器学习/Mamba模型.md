...
translation_route: en/machine-learning/mamba-models
...
> [!warning] 实现与运行边界
> 本页解释概念，不是 Windows 安装教程。上游 `state-spaces/mamba` 仓库在 2026-07-22 列出的核心要求是 Linux、Python 3.10+、PyTorch 1.12+；CUDA selective-scan 扩展还需额外的 GPU/CUDA 条件，且安装模式会影响是否编译该扩展。没有在本课程环境中安装或运行它，不能从架构图推断任意机器都能复现论文吞吐。
...
| 长度复杂度 | 标准 full attention 通常是 $O(L^2)$ | 序列长度方向近似线性 |
...
这个对比不是说 Mamba 一定优于 Transformer。更准确的说法是：Mamba 用压缩状态换取长序列效率；Transformer 用显式 token-token 交互换取更直接的全局建模能力。渐近复杂度也不等于端到端延迟、吞吐或成本：实现、硬件、序列形状、batch、精度、缓存和任务质量都要实测。
...
每来一个 token，只更新相对序列长度固定的状态。因此它的推理状态不会像 KV cache 那样随上下文长度线性膨胀。代价是：历史信息已经被压缩进状态，不能像 attention 那样随时回看所有 token 的原始表示。
...
在比较架构前，先掌握 [[机器学习/00-目录|机器学习]] 的数据边界、基线与指标，以及 [[深度学习/00-目录|深度学习]] 的训练循环、损失和注意力。架构论文的效率结论不替代目标任务上的质量、成本、安全和可靠性评测。

## 参考资料

核验日期：**2026-07-22**。论文结论应限定在其数据、规模和实验设置；上游实现接口与依赖以运行当天文档为准。
