...
translation_route: en/ai-safety/03-project-and-self-assessment/07-project-offline-agent-threat-review
...
输入显式声明资产分类、信任边界、非权威来源、身份 scope/有效期、工具执行身份/副作用/具体 endpoint 与资源、数据出站、依赖摘要、审批/沙箱/记忆流和风险政策。严格契约拒绝重复 JSON key、`NaN`、未知字段、悬空引用、矛盾的 mode/effect、宽泛目的地和未覆盖严重度，避免“脏输入被静默解释”。
...
```powershell
Set-Location '.\examples' # 进入场景、实现和测试所在目录，以下相对路径才可直接运行。

# 漏洞场景：退出码 1，action=BLOCK
python -B .\threat_review.py --scenario .\agent_scenario_vulnerable.json # 运行故意宽权限的场景，验证其稳定得到 BLOCK。

# 加固场景：退出码 0，action=PASS
python -B .\threat_review.py --scenario .\agent_scenario_hardened.json # 运行最小权限的加固场景，验证其得到 PASS。

# 输入契约错误：退出码 2
python -B .\threat_review.py --scenario .\agent_scenario_contract_error.json # 用未知字段 fixture 验证输入契约 fail-closed。

# 内置冒烟检查
python -B .\threat_review.py --self-test # 执行脚本内置的快速自检路径。

# 80 项测试；再用 -O 和 warnings-as-errors 验证测试不依赖 assert/警告
python -B -m unittest discover -s . -p 'test_*.py' -v # 正常模式下发现并详细运行全部测试。
python -B -O -m unittest discover -s . -p 'test_*.py' # 用 -O 验证生产规则不依赖 bare assert。
python -B -W error -m unittest discover -s . -p 'test_*.py' # 将 warning 变成失败，检查更严格的运行条件。
```
...
这 11 条规则不是 OWASP Agentic Top 10 的自动合规映射。它们没有完整验证 Agent 间消息身份/完整性、跨 Agent 级联、人的信任校准或自主偏离/rogue 行为；多 Agent 项目应把这些风险另建场景、终态断言与演练，而不是用本项目的 `PASS` 代替。可将协作侧的负向合同接到 [[多Agent协作/02-工程与质量/08-身份、授权与跨边界信任|身份、授权与跨边界信任]]，再把两类结果共同放入发布门。
...
- [OWASP LLM01:2025 Prompt Injection](https://genai.owasp.org/llmrisk/llm01-prompt-injection/)（获取日期：2026-07-22）
...
- [MITRE ATLAS](https://atlas.mitre.org/)（获取日期：2026-07-22）
